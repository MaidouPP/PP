...
class path {
 ...
 public:
  ...
  static bool generateShortestPath(imdb& db, string& target, string& source);

  static bool generateNewPath(imdb& db, list<path>& partialPaths, path& currentPath, set<film>& previouslySeenFilms, set<string>& previouslySeenActors, string& target);
